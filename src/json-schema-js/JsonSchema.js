...
JsonSchema.prototype.resolveURI = function(schema_stack, uri) {
    var curschema, components, hash_idx,http_idx;
    var name=uri;//name默认等于uri

    hash_idx = uri.indexOf('#');//json reference
    ...
    if (hash_idx !==0) {
        ...
        http_idx=name.indexOf('http://');//是否指向http schema，这里需要访问扩展信息管理服务
    ...
    if (uri.charAt(0) === '/') { //格式为/abc/def
        uri = uri.substr(1);
        curschema = schema_stack[0];//指向栈底
        components = uri.split('/');// [abc,def]
 ...
 * @param object instance / json data
 * @param options
 * @returns {any}
 */
JsonSchema.prototype.validate = function(object, schema, options) {
    var schema_stack = [schema], errors = null, object_stack = [{object: {'__root__': object}, key: '__root__'}];
    ...
    var createResult = function(valid, errors){
        return {
            "valid":valid,
            "errors":errors
        }
    ...
    if (errors)
        return createResult(false, errors.hasOwnProperty('schema') ? errors.schema : errors);
...
JsonSchema.prototype.checkValidity = function(schema_stack, object_stack, options){
    ...
    //临时变量区
    //i用于for循环 整形参数，len与i配合使用，p代表for循环属性参数,objerr代表当前属性检查到的错误
    var i,len,p,objerr;
    ...
    if(schema.hasOwnProperty("type")){
        if(typeof schema.type==="string"){
            ...
            if (options.useCoerce && this.coerceType.hasOwnProperty(schema.type)){
                prop=object[name]=this.coerceType[schema.type](prop);
            }
            ...
            malformed=true;
            for(var i=0,len=schema.type.length;i<len && malformed;i++){
        ...
        if (hasProp || hasPatternProp) {
            ...
            while (i--) {
                ...
                if (hasProp && schema.properties.hasOwnProperty(props[i])) {
                    matched = true;
                    objerr = this.checkValidity(schema_stack.concat(schema.properties[props[i]]), object_stack.concat({object: prop, key: props[i]}), options);
                    if (objerr !== null) {
                ...
                if (hasPatternProp) {
                    for (p in schema.patternProperties)
                        if (schema.patternProperties.hasOwnProperty(p) && props[i].match(p)) {
                            matched = true;
                            objerr = this.checkValidity(schema_stack.concat(schema.patternProperties[p]), object_stack.concat({object: prop, key: props[i]}), options);
                            if (objerr !== null) {
                                objerrs[props[i]] = objerr;
                                malformed = true;
                            }
        ...
        if (options.removeAdditional && hasProp && schema.additionalProperties !== true && typeof schema.additionalProperties !== 'object') {
            ...
            if (schema.hasOwnProperty('additionalProperties')) {
                ...
                if (typeof schema.additionalProperties === 'boolean') {
                    ...
                    //递归遍历所有属性
                    for (i = 0, len = props.length; i < len; i++) {
                        objerr = this.checkValidity(schema_stack.concat(schema.additionalProperties), object_stack.concat({object: prop, key: props[i]}), options);
                        if (objerr !== null) {
                            objerrs[props[i]] = objerr;
                            malformed = true;
                        }
        ...
        if (schema.hasOwnProperty('items')) {
            if (Array.isArray(schema.items)) {
                for (i = 0, len = schema.items.length; i < len; i++) {
                    objerr = this.checkValidity(schema_stack.concat(schema.items[i]), object_stack.concat({object: prop, key: i}), options);
                    if (objerr !== null) {
                        objerrs[i] = objerr;
                        malformed = true;
                    }
                ...
                if (prop.length > len && schema.hasOwnProperty('additionalItems')) {
                    if (typeof schema.additionalItems === 'boolean') {
                        ...
                        for (i = len, len = prop.length; i < len; i++) {
                            objerr = this.checkValidity(schema_stack.concat(schema.additionalItems), object_stack.concat({object: prop, key: i}), options);
                            if (objerr !== null) {
                                objerrs[i] = objerr;
                                malformed = true;
                            }
                ...
                for (i = 0, len = prop.length; i < len; i++) {
                    objerr = this.checkValidity(schema_stack.concat(schema.items), object_stack.concat({object: prop, key: i}), options);
                    if (objerr !== null) {
                        objerrs[i] = objerr;
                        malformed = true;
                    }
            ...
            if (typeof schema.additionalItems !== 'boolean') {
                for (i = 0, len = prop.length; i < len; i++) {
                    var objerr = this.checkValidity(schema_stack.concat(schema.additionalItems), object_stack.concat({object: prop, key: i}), options);
                    if (objerr !== null) {
                        objerrs[i] = objerr;
                        malformed = true;
                    }
...
JsonSchema.prototype.getDefaultInstance = function(schema){
    var object_stack= [this.buildObjectStackItem(null,ROOT_KEY_NAME)];
    var schema_stack;
...
JsonSchema.prototype.generateDefaultValue = function(schema_stack, object_stack){
    var sl=schema_stack.length-1,ol=object_stack.length-1;
    var schema=schema_stack[sl],object=object_stack[ol].object;
    var name=object_stack[ol].key,prop=null;//prop指向当前处理的对象，其实返回时才需要使用
    var uicontrol=schema["uicontrol"];//获取uicontrol信息,dont care any type
    var parentSchema=null;
    ...
    if(schema.hasOwnProperty("type")){
        ...
        if(typeof schema.type==="string"){
            ...
            if (schema.type==="object" && schema.properties && typeof schema.properties==="object"){
                for(var v in schema.properties){

                    var new_schema_stack=clone(schema_stack);
                    //如果出现$ref，则生成新的schema_stack，并重新处理
                    ...
                    var new_schema=new_schema_stack[new_schema_stack.length-1];
                    if(schema.properties[v].hasOwnProperty("$ref")){
